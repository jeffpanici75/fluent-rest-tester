...
import { new_api } from './test_config';

let config = yaml.load('integration-tests.yml');

Object.keys(config.plural || {}).forEach(x => {
    pluralize.addPluralRule(x, config.plural[x]);
});

Object.keys(config.singular || {}).forEach(x => {
    pluralize.addSingularRule(x, config.singular[x]);
});
...
function create_resource_defs(resources) {
...
function get_dependent_resources(all_defs, def, resources) {
    let keys = Object.keys(def.fields);
    keys.forEach(k => {
        let f = def.fields[k];
        if (!f.from || f.from === '<parent>')
            return;
        let func = new_api[f.from];
        if (!func)
            throw new Error(`No client proxy for ${f.from}.`);
        let target_def = all_defs.find(x => x.name === f.from);
        if (!target_def)
            throw new Error(`No resource_def for ${f.from}.`);
        resources.unshift({ func, def: target_def, id_name: f.id || 'id' });
        get_dependent_resources(all_defs, target_def, resources);
    });
}

async function create_dependent_resources(all_defs, def, deps, done) {
    let resources = [];
    get_dependent_resources(all_defs, def, resources);
...
function delete_dependent_resources(deps, done) {
    if (!deps) {
        done();
        return;
    }
...
function test_resource_defs(all_defs, defs, api) {
    if (!api)
        throw new Error('test_resource_defs requires api parameter.');
    ...
    defs.forEach(x => {
        ...
        describe(`Resource '${x.name}' HTTP verbs`, function () {
            ...
            before(done => {
                ...
                deps = {};
                create_dependent_resources(all_defs, x, deps, done);
            });
                        ...
                        describe('pre-existing resources', () => {
                            ...
                            it('should return all other pages', done => {
                                function walk_pages(n) {
                                    ...
                                    new_api.resource_at(first_page._links.pages[n].href)
                                        .then(result => {
                                            should.exist(result);
                                            should.exist(result.response);
                                            should.exist(result.resource);
...
let all_defs = create_resource_defs(config.resources);
test_resource_defs(all_defs, all_defs, new_api); 
